...
1. **מקומי** — `localStorage`, קובץ שיובא לדפדפן הזה בלבד
2. **משותף** — `/api/question-bank`, מה שהמנהל פרסם לכל התלמידים
3. **מובנה** — `client/public/question_bank_infinitycloser.json`, נבנה עם האתר
...
מסך הבית ומסך הניהול מציגים תמיד איזה מקור פעיל, כי המקום היחיד שבו
המנהל והתלמידים יכולים לראות שאלות שונות הוא מאגר מקומי שמסתיר מאגר
משותף — ועל כך יש התרעה מפורשת.

### מאגר משותף

בפריסת Vercel המנהל מעלה JSON פעם אחת מ**ניהול → תוכן → פרסום לכל
התלמידים**, והוא נשמר ב-Vercel Blob ונקרא על ידי כולם. הכתיבה מוגנת
ב-`ADMIN_UPLOAD_TOKEN` שנבדק **בשרת** — להבדיל מהכניסה למערכת, שהיא
שער תצוגה בצד לקוח.

בלי Blob store מוגדר, הכל ממשיך לעבוד בדיוק כמו קודם.

ראה [`docs/DEPLOY-VERCEL.md`](docs/DEPLOY-VERCEL.md).
...
**Vercel** (מומלץ — תומך במאגר משותף):
[`docs/DEPLOY-VERCEL.md`](docs/DEPLOY-VERCEL.md).

**GitHub Pages**: `.github/workflows/deploy-pages.yml` רץ על כל push
ל-`main` — בדיקת טיפוסים → בדיקות → build → פריסה. Build שנכשל לא
מתפרסם. אין שם שרת, ולכן אין מאגר משותף: כל מנהל מייבא לדפדפן שלו.
לפרטים ראה [`GITHUB_PAGES_SETUP.md`](GITHUB_PAGES_SETUP.md).
...
- **אין סנכרון בין מכשירים.** משתמשים והיסטוריית אימונים נשמרים
  ...
  מאגר השאלות הוא היוצא מן הכלל היחיד — הוא משותף (ב-Vercel).
